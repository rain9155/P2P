...
public class StatusbarUtils {
    ...
    public static void immersive(Window window, int color) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            setTranslucentView((ViewGroup)window.getDecorView(), color);
        }
    }
     ...
     */
    public static void immersiveInFragments(AppCompatActivity activity, int color) {
        if(Build.VERSION.SDK_INT < MIN_API) return;
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            setTranslucentView((ViewGroup)activity.getWindow().getDecorView(), color);
        }
    }
    ...
    private static void setTranslucentView(ViewGroup container, int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            if (translucentView == null && color != 0) {
    ...
    public static void darkMode(AppCompatActivity activity) {
        darkMode(activity.getWindow(), DEFAULT_COLOR);
    }

    /**
     * 设置状态栏darkMode,字体颜色及icon变黑(目前支持MIUI6以上,Flyme4以上,Android M以上)
     */
    public static void darkMode(Window window, int color) {
        if (isFlyme4Later()) {
