...
public class IndicatorView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        int wrapWidth = mIndicatorCount * mIndicatorRadius * 2 + mIndicatorOffset * (mIndicatorCount - 1) + getPaddingEnd() + getPaddingStart();
        int wrapHeight = mIndicatorRadius * 2 + getPaddingBottom() + getPaddingTop();
    ...
    private void init(AttributeSet attrs) {
        ...
        mIndicatorRadius = typedArray.getDimensionPixelSize(R.styleable.IndicatorView_indicatorRadius, 10);
        mIndicatorOffset = typedArray.getDimensionPixelSize(R.styleable.IndicatorView_indicatorOffset, 8);
    ...
    protected void onDraw(Canvas canvas) {
        int y = mHeight / 2;
        int x = mIndicatorRadius + (mWidth - (mIndicatorCount * mIndicatorRadius * 2 + mIndicatorOffset * (mIndicatorCount - 1))) / 2;//居中
        for(int i = 0; i < mIndicatorCount; i++){
            if(i == mCurrentIndicator) mPaint.setColor(mIndicatorColorSelected);
            else mPaint.setColor(mIndicatorColor);
